...
　　・【既知情報は再確認しない】コールセンター/事案入力で既に判明している事項（申告済みの
　　　エラーコード、機種、申告症状など）は、ヒアリング項目として再度確認しない。標準項目の中に
　　　既知の観点が含まれる場合、その観点（行）は除く。観点の一部だけが既知なら、未確認の観点だけ残す。
　　　例: エラーコードは既に聞いているので「表示されているエラーコードは何か」は確認しない
　　　（「申告以外のエラーが併発していないか」など未確認の点は残す）。
